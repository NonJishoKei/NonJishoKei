...
CLipboradText = pyperclip.paste()

IndexTextList = []
with open(r"v3_index.txt", 'r', encoding='utf-8') as f:
    IndexText = f.readlines()
    for i in IndexText:
        IndexTextList.append(i.replace('\n', ''))

InputText = CLipboradText.replace('\n', '')
...
if LastLetter in NoNeedProcess:
    ...
    print('至少需要处理2次')
    ProcessText = InputText  # 尝试在原型中进行查找
    ...
    if SearchInIndex(ProcessText) == False:
        print("找不到"+InputText + "原型")
CLipboradText = ProcessText
pyperclip.copy(CLipboradText)
